...
public class InventoryDeductListener implements RocketMQListener<String> {
    ...
    public void onMessage(String orderJson) {
        try {
            com.cloud.mail.api.order.model.Order order = JSON.parseObject(orderJson, com.cloud.mail.api.order.model.Order.class);
            ...
            List<String> items = stringRedisTemplate.opsForList().range(orderItemsKey, 0, -1);
